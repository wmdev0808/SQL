...
-- [Alternative, correct one]
WITH ordered_orders
AS (
	SELECT 
		total_amt_usd, 
		ROW_NUMBER() OVER (ORDER BY total_amt_usd) row_id,
...
SELECT we.occurred_at event_date, we.channel channel_name, a.name account_name
FROM web_events we
JOIN accounts a
	ON we.account_id = a.id
	AND we.occurred_at = (SELECT MAX(occurred_at) FROM web_events)
...
SELECT 
	sr.name, 
	COUNT(1) total_number_of_orders, 
	SUM(o.total_amt_usd) total_sales,
	CASE
		WHEN COUNT(1) > 200 OR SUM(o.total_amt_usd) > 750000 THEN 'top'
		WHEN COUNT(1) > 150 OR SUM(o.total_amt_usd) > 500000 THEN 'middle'
		ELSE 'low'
